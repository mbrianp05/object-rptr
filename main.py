...
class Person:
  ...
  def setGroup(self, group: int):
    if group < 11:
      raise Exception("El número del grupo debe ser como mínimo 11")

    group = str(group)

    if len(group) != 2:
      raise Exception("El grupo debe tener 2 cifras")

    if int(group[0]) > 4:
      raise Exception("El grupo empieza por el año por lo que la primera cifra no puede ser mayor que 4")

    if int(group[1]) == 0:
      raise Exception("El segundo dígito del grupo debe ser mayor que 0")
  ...
  def setTeam(self, team: int):
    if team < 1:
      raise Exception("El número del equipo debe ser como mínimo 1")
...
def print_stylistic_list(title: str, items: List[Any]):
  first_character = "┌"
  in_between_char = "│"
  final_character = "└"
  list_decorator = "◊"
